...
// import { auth } from "../firebase"; // auth is not used directly in this component
...
const getTechnologyById = async (id) => {
    ...
    if (!response.ok) {
        ...
        if (response.status === 404) errorMsg = "Technology not found.";
             ...
             // No need to clear local storage here, let App.jsx or ProtectedRoute handle it
        }
...
const updateTechnologyAPI = async (id, updatedData) => {
    const token = getTokenFromStorage();
    if (!token) throw new Error("Authentication token not found. Please log in.");
    
    // Permission check should ideally be done before calling this,
    // but as a safeguard or if called directly elsewhere.
    // The server-side will enforce this regardless.
    ...
    Object.entries(updatedData).forEach(([key, value]) => {
        if (key === "images" || key === "newImageData") return;
    ...
    if (updatedData.newImageData && updatedData.newImageData.length > 0) {
        updatedData.newImageData.forEach((imgData, index) => {
            if (imgData.file) {
                ...
                // Backend expects imageCaptions[index] for new files if multer is configured for it
                // If not, adjust backend or how captions are sent.
                // For simplicity, assuming backend handles captions associated with new files correctly.
                // If specific caption mapping is needed for new files, ensure backend supports it.
                // The provided server.js `processNewUploadedFiles` uses `bodyForCaptions[`imageCaptions[${i}]`]`.
                // This means captions for new files should be sent as separate fields if that's the design.
                // Let's assume for now captions are handled with existing images, and new image captions are not explicitly mapped here.
                // If they are, they need to be added to formData.
            }
        });
    }
...
const EditTechnology = () => {
    ...
    const [isPageAccessible, setIsPageAccessible] = useState(false); // Determines if form is shown
    ...
    const fetchTechDetails = useCallback(async () => {
        if (!id) {
            setPageError("Technology ID not found.");
            setLoading(false);
            setIsPageAccessible(false);
            return;
        }
        if (!canUserEdit) { // Check permission before fetching
    ...
    useEffect(() => {
        ...
        if (currentUserInfo && token) {
            ...
            // Permission check and fetch will be triggered by canUserEdit changing or id changing
        } else {
    ...
    useEffect(() => {
        if (userInfo) { // Only fetch if userInfo is loaded
            fetchTechDetails();
        }
    }, [userInfo, fetchTechDetails]); // Re-fetch if userInfo changes (e.g. on refresh and re-auth)
    ...
    // const handleSwitchChange = useCallback((e) => setTechData(prev => ({ ...prev, [e.target.name]: e.target.checked })), []); // Not used for radio
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!canUserEdit) { // Double check permission before submitting
            toast.error("Permission Denied. You cannot update technologies.");
            setPageError("Permission Denied.");
            return;
        }
        ...
        try {
            await updateTechnologyAPI(id, payload);
                ...
                <form onSubmit={handleSubmit} noValidate>
                    <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}><Box sx={{ border: `1px solid ${theme.palette.divider}`, p: 2.5, borderRadius: 2 }}><Typography variant="h6" gutterBottom sx={{ fontWeight: 500, mb: 1.5 }}>Images</Typography>
                            ...
                            <Button variant="outlined" component="label" startIcon={<AddPhotoAlternateIcon />} sx={{ mt: 1, textTransform:'none' }} disabled={submitting || ((techData.images?.length || 0) + newImages.length >= 5)}>Add New Images (Max 5 Total)<input type="file" accept="image/*" multiple hidden ref={fileInputRef} onChange={handleImageUpload} /></Button>
                        </Box></Grid>
