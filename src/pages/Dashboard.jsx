...
const Dashboard = () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 401 || response.status === 403) {
                    errorMsg = result.message || "Access Denied. Your session might have expired.";
    ...
    const {
        totalTechs, uniqueGenresCount, totalInnovators,
        totalApplications, totalUseCases, genreCounts, innovatorsTechMap,
    } = useMemo(() => {
        const genreMap = {};
        const innovatorMap = {};
        let appCount = 0;
            ...
            if (tech?.innovators) {
                if (Array.isArray(tech.innovators)) {
                    ...
                    tech.innovators.split(/[\/,]/).forEach((name) => {
                        ...
                        if (trimmed) innovatorMap[trimmed] = (innovatorMap[trimmed] || 0) + 1;
                    });
                }
            }
        ...
        <Card elevation={0} sx={{ height: "100%", background: "rgba(255, 255, 255, 0.8)", backdropFilter: "blur(10px)", border: "1px solid rgba(230, 230, 230, 0.5)", borderRadius: "16px", p: 1, transition: "transform 0.2s ease-in-out", wordBreak: "break-word", "&:hover": { transform: "translateY(-4px)" } }}>
            <CardContent sx={{ p: 1 }}>
                <Box sx={{ display: "flex", alignItems: "center", mb: 0.5 }}>
                    <Box sx={{ p: 0.5, borderRadius: "12px", backgroundColor: `${color}15`, display: "flex", alignItems: "center", justifyContent: "center", mr: 0.5 }}>
    ...
    const filteredData = useMemo(() => (data || []).filter((item) => {
        ...
        if (filterInnovators && item.innovators) {
            ...
            if (Array.isArray(item.innovators)) {
                innovatorMatch = item.innovators.some((innovator) => innovator?.name?.toLowerCase().includes(filterInnovators.toLowerCase()));
            } else if (typeof item.innovators === "string") {
                innovatorMatch = item.innovators.toLowerCase().includes(filterInnovators.toLowerCase());
            }
            if (!innovatorMatch) return false;
        }
        return true;
    }), [data, searchQuery, filterGenre, filterInnovators]);
    ...
    const requestEditTechnology = (techId) => {
        if (canEditTech) {
            ...
            toast.error("You do not have permission to edit technologies.", { position: "top-center" });
        }
    };
    ...
    const requestDeleteTechnology = (id) => {
        if (!canDeleteTech) {
            toast.error("You do not have permission to delete technologies.", { position: "top-center" });
            return;
        }
        setTechToDelete(id);
    ...
    const confirmDeleteTechnology = async () => {
        if (!techToDelete || !canDeleteTech) {
            toast.error("Action not allowed or item not selected.", { position: "top-center" });
    ...
    const handleLogout = async () => {
        try {
            ...
            localStorage.clear();
            setIsAuthenticated(false); setUserInfo(null); setData([]); navigate("/login");
        }
    };
    ...
    if (!loading && !isAuthenticated && window.location.pathname !== "/login") {
        return (
            <Layout title="Dashboard - Login Required">
                ...
                <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "calc(100vh - 150px)" }}>
    ...
    return (
        <Layout title="Admin Dashboard">
            <ToastContainer position="top-center" autoClose={3000} />
            <Box sx={{ background: "#F5F5F5", p: { xs: 1, sm: 2 }, minHeight: 'calc(100vh - 64px)' }}>
                <Box sx={{ maxWidth: "1200px", mx: "auto" }}>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 4, px: 2, wordBreak: "break-word" }}>
                        ...
                        <Button variant="outlined" color="error" startIcon={<LogoutIcon />} onClick={handleLogout} sx={{ textTransform: "none" }}>Logout</Button>
                    </Box>

                    {loading ? (
                        <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "50vh" }}><CircularProgress size={60} /></Box>
                    ) : error ? (
                        <Paper sx={{ p: 3, bgcolor: "#fff3f3", color: "error.main", borderRadius: 2, mx: 2 }}>
                            <Typography>Error loading dashboard data: {error}</Typography>
                            <Button onClick={getAllTechs} sx={{ mt: 1 }}>Retry</Button>
                        </Paper>
                    ) : (
                        <>
                            <Box sx={{ px: 2 }}>
                                                    ...
                                                    <Box sx={{ maxHeight: 300, overflowY: "auto" }}>
                                                        {Object.entries(genreCounts).map(([genre, count], index) => (
                                                            <Box key={genre} sx={{ mb: 2 }}>
                                                                <Box sx={{ display: "flex", justifyContent: "space-between", mb: 1 }}><Typography variant="body2" color="text.secondary">{genre}</Typography><Typography variant="body2" fontWeight="500">{count}</Typography></Box>
                                                                <Box sx={{ width: "100%", height: "6px", borderRadius: "3px", bgcolor: "grey.100", overflow: "hidden" }}><Box sx={{ width: `${(count / (totalTechs || 1)) * 100}%`, height: "100%", bgcolor: `hsl(${index * 137.5}, 70%, 50%)`, transition: "width 0.5s ease-in-out" }} /></Box>
                                                            </Box>
                                                        ))}
                                                    </Box>
                                                </CardContent>
                            ...
                            <Box sx={{ mb: 4, mt: 2, px: 2, wordBreak: "break-word" }}>
                                <Box sx={{ mb: 2 }}>
                                    <Typography variant="h4" component="h1" sx={{ fontWeight: 700, mb: 1, background: "linear-gradient(45deg, #141E30, #243B55)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Technology Innovations</Typography>
                                    <Typography variant="subtitle1" color="text.secondary">Explore and manage cutting-edge technologies</Typography>
                                </Box>
                                <Grid container spacing={2} alignItems="center" sx={{ mb: 2 }}>
                                    <Grid item xs={12} sm={true}>
                                        <Paper component="form" onSubmit={(e) => e.preventDefault()} sx={{ p: "4px 8px", display: "flex", alignItems: "center", borderRadius: 2, boxShadow: "none", border: `1px solid ${theme.palette.divider}`, transition: "border-color 0.3s", "&:hover": { borderColor: theme.palette.text.primary }, minHeight: consistentHeight }}>
                                ...
                                <Fade in={!loading}>
                                    <Box>
                                        {currentPageData.length > 0 ? (
                                            <TechList techs={currentPageData} onDeleteTech={requestDeleteTechnology} onEditTech={requestEditTechnology} onShowDetails={requestShowDetails} canEdit={canEditTech} canDelete={canDeleteTech} />
                                        ) : (
                                        ...
                                        {totalPages > 1 && (
                                            <Box sx={{ display: "flex", justifyContent: "center", mt: 6 }}><Pagination count={totalPages} page={page} onChange={handlePageChange} color="primary" size={isMobile ? "small" : "large"} shape="rounded" /></Box>
                                        )}
                                    </Box>
                                </Fade>
                            </Box>
                    ...
                    <Dialog open={openDeleteDialog} onClose={cancelDeleteTechnology} aria-labelledby="delete-confirmation-dialog" PaperProps={{ sx: { borderRadius: 2, backgroundColor: "#f6f8fa", boxShadow: "0 4px 12px rgba(27,31,35,0.15)", border: "1px solid #d1d5da", maxWidth: "400px", m: 2 } }}>
                        ...
                        <DialogActions sx={{ pt: 2, pb: 2, px: 3 }}>
                            ...
                            <Button onClick={confirmDeleteTechnology} variant="contained" sx={{ textTransform: "none", fontWeight: 500, backgroundColor: "black", color: "white", "&:hover": { backgroundColor: "grey" } }} autoFocus>Delete</Button>
                        </DialogActions>
                    </Dialog>
                </Box>
            </Box>
