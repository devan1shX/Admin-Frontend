...
const Dashboard = () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 401 || response.status === 403) {
                    errorMsg = "Access Denied fetching technologies. Your session might have expired.";
                    setIsAuthenticated(false);
                    setUserRole(null);
                    localStorage.removeItem("user");
                    localStorage.removeItem("token");
    ...
    const StatCard = ({ icon: Icon, title, value, subtitle, color }) => (
        <Card
            elevation={0}
            sx={{
                height: "100%", background: "rgba(255, 255, 255, 0.8)", backdropFilter: "blur(10px)",
                border: "1px solid rgba(230, 230, 230, 0.5)", borderRadius: "16px", p: 1,
                transition: "transform 0.2s ease-in-out", wordBreak: "break-word",
    ...
    const filteredData = data.filter((item) => {
        ...
        if (filterGenre && item.genre?.toLowerCase() !== filterGenre.toLowerCase()) {
            return false;
        }
        if (filterInnovators && item.innovators) {
            let innovatorMatch = false;
            if (Array.isArray(item.innovators)) {
                innovatorMatch = item.innovators.some(
                    (innovator) => innovator?.name?.toLowerCase().includes(filterInnovators.toLowerCase())
                );
    ...
    const requestEditTechnology = (techId) => {
        if (isAdmin) {
            ...
            toast.error("Permission denied. Employees cannot edit technologies.", { position: "top-center" });
        } else {
    ...
    const requestDeleteTechnology = (id) => {
        if (!isAdmin) {
            toast.error("Permission denied. Only Admins can delete technologies.", { position: "top-center" });
            return;
        }
        setTechToDelete(id);
    ...
    const confirmDeleteTechnology = async () => {
        if (!techToDelete || !isAdmin) {
            ...
            setOpenDeleteDialog(false); setTechToDelete(null); return;
        }
        try {
        ...
        <Layout title="Admin Dashboard">
            ...
            <Box sx={{ background: "#F5F5F5", p: { xs: 1, sm: 2 } }}>
                <Box sx={{ maxWidth: "1200px", mx: "auto" }}>
                                ...
                                <Grid container spacing={2} alignItems="center" sx={{ mb: 2 }}>
                                    <Grid item xs={12} sm={true}>
                                        <Paper component="form" onSubmit={(e) => e.preventDefault()} sx={{ p: "4px 8px", display: "flex", alignItems: "center", borderRadius: 2, boxShadow: "none", border: `1px solid ${theme.palette.divider}`, transition: "border-color 0.3s", "&:hover": { borderColor: theme.palette.text.primary }, minHeight: consistentHeight }}>
                                            ...
                                            <TextField
                                                ...
                                                InputProps={{
                                                    disableUnderline: true, sx: { ml: 1 }, endAdornment: (
                                    ...
                                    <Grid item xs={12} sm={"auto"}>
                                        <Button
                                            ...
                                            fullWidth={isMobile}
                                            sx={{ height: consistentHeight, borderRadius: 2, background: "linear-gradient(90deg, #141E30, #243B55)", boxShadow: "0 4px 8px rgba(0,0,0,0.1)", transition: "all 0.3s", "&:hover": { boxShadow: "0 6px 12px rgba(0,0,0,0.2)", transform: "translateY(-2px)" } }}
                                        >
                                            Add Technology
                                        </Button>
                                    </Grid>
                    ...
                    <Dialog
                        open={openDeleteDialog} onClose={cancelDeleteTechnology}
                    ...
                    >
                        ...
                        <DialogActions sx={{ pt: 2, pb: 2, px: 3 }}>
                            ...
                            <Button onClick={confirmDeleteTechnology} variant="contained" color="error" sx={{ textTransform: "none", fontWeight: 500, backgroundColor: "black", "&:hover": { backgroundColor: "gray" } }}>
                                Delete
                            </Button>
                        </DialogActions>
                    </Dialog>
