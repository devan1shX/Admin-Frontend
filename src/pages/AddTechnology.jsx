...
const addTechnologyAPI = async (newData) => {
    ...
    Object.entries(newData).forEach(([key, value]) => {
        if (key === 'imagesData') return; // Files are handled separately if 'imagesData' holds file objects
    ...
    if (newData.imagesData && newData.imagesData.length > 0) {
        newData.imagesData.forEach((imageObj, index) => {
            if (imageObj.file) {
                formData.append('images', imageObj.file);
                formData.append(`imageCaptions[${index}]`, imageObj.caption || "");
            }
        });
    }
...
const AddTechnology = () => {
    ...
    const [loadingPage, setLoadingPage] = useState(true); // For initial permission check
    ...
    const [addData, setAddData] = useState({
        ...
        imagesData: [], // Store { file, preview, caption } objects
    ...
    const handleImageUpload = (e) => {
        ...
        const limit = 5;
        const remainingSlots = limit - currentImageCount;
        
        if (files.length > remainingSlots) {
            toast.warn(`You can only add ${remainingSlots > 0 ? remainingSlots : 0} more image(s). Max ${limit} images allowed.`);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        // The imagesData already contains { file, caption }
        // The addTechnologyAPI function expects 'imagesData' for this.
                        ...
                        <Grid item xs={12}>
                            <Box sx={{ border: `1px solid ${theme.palette.divider}`, p: 2.5, borderRadius: 2 }}>
                                ...
                                <label htmlFor="image-upload-input">
                                    <Button variant="outlined" component="span" startIcon={<AddPhotoAlternateIcon />} sx={{ mb: 2, textTransform:'none' }} disabled={isSubmitting || addData.imagesData.length >= 5} > Select Images </Button>
                                </label>
